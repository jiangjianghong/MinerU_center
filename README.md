...
项目使用 SQLite 保存配置、实例和任务历史：

```text
宿主机：    <项目目录>/data/mineru_center.db
容器内：    /app/data/mineru_center.db
Compose：   ./data:/app/data
```

使用同一个项目目录执行代码或镜像更新时，数据不会因为容器重建而丢失。
任务列表中的“请求端 IP”记录提交任务的客户端地址；在反向代理后运行时优先使用 `X-Forwarded-For`。
不要删除 `data` 目录，也不要在没有复制该目录的情况下切换部署目录。应用启动时会将无法恢复的遗留排队/处理中任务自动标记为失败。
...
| 方法 | 路径 | 说明 |
|------|------|------|
| GET | `/api/stats` | 获取队列、任务和实例统计快照（前端自适应轮询） |
